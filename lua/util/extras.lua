...
function X.new()
  ...
  self.float = Float.new({ title = "Extras" })
...
function X:render()
  self.text:nl():nl():append("Extras", "LazyH1"):nl():nl()
  self.text
    :append("This is a list of all enabled/disabled extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  if not extra.managed then
    self:diagnostic({
      message = "Not managed by Extras (config)",
  ...
  if extra.source.name ~= "NeoVim" then
